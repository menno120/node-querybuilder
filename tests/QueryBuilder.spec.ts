...
describe("QueryBuilder", () => {
	...
	describe("constructor", () => {
		...
		it("should be in debug mode", () => {
			expect(new QueryBuilder(true).debugging).to.equal(true);
		});

		it("shouldn't be in debug mode", () => {
			expect(new QueryBuilder(false).debugging).to.equal(false);
		});
	});
	...
	describe("select", () => {
		it("type should be select", () => {
			...
			expect(query.get().type).to.equal(QueryType.select);
		});
	});
	...
	describe("insert", () => {
		it("type should be insert", () => {
			...
			expect(query.get().type).to.equal(QueryType.insert);
		});
	});

	describe("update", () => {
		it("should return a valid SQL", () => {
			let query = new QueryBuilder().update("tablename", ["id"], []);
			expect(query.get().type).to.equal(QueryType.update);
		});
	});
	...
	describe("delete", () => {
		it("type should be delete", () => {
			...
			expect(query.get().type).to.equal(QueryType.delete);
		});
	});
	...
	describe("truncate", () => {
		it("type should be truncate", () => {
			...
			expect(query.get().type).to.equal(QueryType.truncate);
		});
	});
});
